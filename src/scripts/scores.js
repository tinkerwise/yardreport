...
import {
  ...
  fetchBoxscoreRaw,
  fetchLiveFeed,
  fetchPitchArsenalRaw,
  fetchTeamHittingStatsRaw,
  fetchPitcherVsTeamRaw,
} from './mlbApi.js';
...
export async function fetchBoxscore(gamePk) {
  ...
  try {
    const data = await fetchBoxscoreRaw(gamePk);
...
export async function fetchArsenal(playerId) {
  ...
  try {
    const data = await fetchPitchArsenalRaw(playerId, SEASON);
...
export async function fetchTeamStats(teamId) {
  ...
  try {
    const data = await fetchTeamHittingStatsRaw(teamId, SEASON);
...
export async function fetchPitcherVsTeam(pitcherId, oppTeamId) {
  ...
  try {
    const data = await fetchPitcherVsTeamRaw(pitcherId, oppTeamId);
    const result = data.stats?.[0]?.splits?.[0]?.stat ?? null;
...
function renderLiveWalkupQueueRow(label, person) {
  ...
  const urls = getWalkupSongUrls(person.id, fullName);
...
function renderLineupRows(team, gameState = 'preview', teamId = null) {
  ...
  if (gameState === 'final' || gameState === 'live') {
...
function renderScoutNotes(game, arsenals, matchupCtx = null) {
  ...
  if (isLive) {
    ...
    if (oriolesBatting) {
      liveMusicRows.push(
        renderLiveWalkupQueueRow('At Bat', batter),
        renderLiveWalkupQueueRow('On Deck', onDeck),
        renderLiveWalkupQueueRow('In Hole', inHole),
      );
    }
    if (oriolesPitching && pitcher?.fullName) {
      const warmupUrls = getWalkupSongUrls(pitcher.id, pitcher.fullName);
    ...
    if (queueRowsHtml) {
      liveWalkupQueue = `<div class="live-music-queue">
        <div class="live-music-head">Orioles Walk-Up Queue</div>
        ${queueRowsHtml}
      </div>`;
    }
...
function renderPreviewTeamCard(game, boxData, side, arsenalData) {
  ...
  const probablePitcherSongs = Number(teamId) === ORIOLES_ID
    ? getWalkupSongUrls(probablePitcherId, probablePitcher)
    : [];
...
function renderBenchRows(team, gameState = 'live') {
  ...
  const rows = benchIds.map(id => {
    ...
    const ss = p.seasonStats?.batting ?? {};
    const cols = [
      ss.avg != null ? formatSlashStat(ss.avg) : '—',
      ss.gamesPlayed ?? '—',
      ss.runs ?? '—',
      ss.hits ?? '—',
      ss.homeRuns ?? '—',
    ...
    return `<div class="score-lineup-row">
      ...
      <span class="score-lineup-box-cols score-lineup-box-cols--bench">${cols}</span>
    </div>`;
  }).filter(Boolean).join('');
  return header + rows;
}
...
export async function loadScores() {
  ...
  try {
    ...
    const scheduleHydrate = 'linescore,team,venue,decisions,probablePitcher';
    const [ydData, todayData, tmData] = await Promise.all([
      fetchScheduleByDate(yesterday, scheduleHydrate),
      fetchScheduleByDate(today, scheduleHydrate),
      fetchScheduleByDate(tomorrow, scheduleHydrate),
    ]);
...
async function updateTicker(gamePk) {
  ...
  if (!el) return;
  try {
    const TICKER_FIELDS = 'liveData,plays,currentPlay,result,description,event,matchup,batter,pitcher,fullName,count,balls,strikes,outs,about,inning,halfInning,gameData,status,abstractGameState';
    const data = await fetchLiveFeed(gamePk, TICKER_FIELDS);
